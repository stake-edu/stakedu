...
contract("StakingRewardPool", (accounts) => {
  ...
  it("starting a new reward phase should setup the reward phase and reward token balance in the pool", async () => {
    ...
    assert.equal(
      phase.reward.toNumber(),
      reward,
      "Invalid reward phase amount",
    );
  ...
  it("claimableReward should return the reward yet to be claimed ", async () => {
    ...
    let stakeReward1 = (
      await pool.claimableReward({ from: accounts[1] })
    ).toNumber();
  ...
  it("The reward for 1 stake across the full reward phase should equal the full reward", async () => {
    ...
    assert.equal(
      Math.round(rewardEarned / contractRewards),
      1,
  ...
  it("The reward of 2 overlapping stakes should be proportional to the amount of tokens staked", async () => {
    ...
    assert.equal(
      Math.round(rewardEarned2 / rewardEarned1),
      2,
  ...
  it("The rewards for 2 non overlapping stakes of the same amount, should be proportional to the time the tokens were staked", async () => {
    ...
    assert.equal(
      ...
      "Reward earned by account1 should be double that of account2",
    );
  });
  ...
  it("The reward of several stakes from 2 accounts should be proportional to the amount and time of the tokens staked", async () => {
    ...
    let expected1Day2 =
      Math.floor((ts2s - ts1s) / 1) + Math.floor(((ts2e - ts2s) * 1) / 4);
    assert.equal(
      reward1Day2,
      expected1Day2,
    ...
    let reward2Day2 = (
      await pool.claimableReward({ from: accounts[2] })
    ).toNumber();
    ...
    let expected1Day3 =
      Math.floor(ts2s - ts1s) + Math.floor(((ts3e1 - ts2s) * 1) / 4);
    let expected2Day3 = Math.floor(((ts3e2 - ts2s) * 3) / 4);

    assert.equal(
      rewardEarned1,
      expected1Day3,
      "Incorrect reward receved by account 1",
    );
    assert.equal(
      rewardEarned2,
      expected2Day3,
      "Incorrect reward receved by account 2",
    );

    let expectedRewardDistributed = ts3e2 - ts1s;
    assert.ok(
      Math.abs(totalRewardDistributed - expectedRewardDistributed) <= 1,

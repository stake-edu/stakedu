...
import { eth, getInstance, myWeb3 } from "./provider";
import { getAccount, toNumber, toTokenDecimals, toTokenUnits } from "./utils";
...
export const getRewardPeriod = async (index) => {
  ...
  return {
    ...
    rewardPerTokenStaked: await toNumber(
      etb,
      rewardPhase.rewardPerTokenStaked,
      4,
    ),
...
export const createRewardPeriod = async (amount, startDate, endDate) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const result = await pool.newRewardPeriod(amountDecimals, start, end, {
        from: account,
      });
      resolve(result);
    } catch (error) {
